...
/* ── Find available rooms/times for a conflicting block ── */
function findSuggestions(conflictBlock, allSchedules) {
  const { day, start, end, room } = conflictBlock;
  const dur = end - start;
  const suggestions = [];

  // 1. Same day, same duration — find free rooms
  ALL_ROOMS.forEach(r => {
    if (r === room) return;
  ...
  // 2. Same room, different time slots (same day) — find free windows
  for (let t = DAY_START; t + dur <= DAY_END; t++) {
    if (t === start) continue;
    if (t < LUNCH_END && t + dur > LUNCH_START) continue; // skip lunch overlap
    const blocked = allSchedules.filter(s =>
...
function ConflictToast({ conflicts, allSchedules, onClose, onMoveSchedule }) {
  ...
  // For each conflict, compute suggestions based on blockA
  ...
  return (
    <div style={{
      ...
      <div style={{fontSize:12,color:"#64748b",marginBottom:12}}>
        Resolve the following conflicts before saving. Suggestions are based on the existing schedule.
      </div>
          ...
          <div key={i} style={{
            ...
            {c.suggestions.length > 0 && (
              <>
                <div style={{fontSize:11,fontWeight:700,color:"#6b7280",marginBottom:6,
                  background:"#f1f5f9",borderRadius:6,padding:"4px 8px",display:"inline-block"}}>
                    ...
                    <div key={si} style={{
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:10,color:"#6b7280"}}>
                          {sg.day} · {fmtRange(sg.start, sg.end)} · {sg.room}
                        </div>
                      </div>
                      {onMoveSchedule && c.blockA && (
                        <button
                          onClick={() => onMoveSchedule(c.blockA, sg)}
                          style={{
                            padding:"5px 12px",
            ...
            {c.suggestions.length === 0 && (
              <div style={{fontSize:11,color:"#f59e0b",background:"#fef9c3",border:"1px solid #fde68a",borderRadius:6,padding:"5px 10px",marginTop:6}}>
                ⚠ No free alternatives found in the same time range. Try a different day or time.
              </div>
            )}
          </div>
        ))}
...
/* ════════════════════════════════════════════════════════════
   STUDENT WEEKLY GRID
   — Subjects dropdown = only what's assigned to that instructor
   — Works for GE Only, Major Only, and Both instructors
   ════════════════════════════════════════════════════════════ */
function StudentWeeklyGrid({ grid, setGrid, theme, activeSemester }) {
  const [instructorList, setInstructorList] = useState([]);
  // assignedSubjects: { normName: [{ subject_name, subject_type }] }
...
useEffect(() => {
    ...
    // Load ALL assignments for this semester upfront, group by instructor
  ...
  const upd = (day, t, field, val) => {
    setGrid(prev => {
      ...
      if (field === "instructor") {
        u.subject = "";
        // no more loadAssignedSubjects needed — already loaded on mount
      }
      return { ...prev, [day]: { ...prev[day], [t]: u } };
    });
  };

  // Group instructors — but only by employment type now (no semester filter in pool needed)
                ...
                <td key={day} style={{padding:"5px 6px",border:`1px solid ${theme.light2}`,verticalAlign:"top",background:sub?(lab?theme.light2:theme.light):"transparent"}}>
                  ...
                  {instructorList.length > 0
                    ? <InstructorSelect
                        ...
                        style={{width:"100%",padding:"4px 6px",border:`1px solid ${theme.border}`,borderRadius:4,fontSize:11,fontWeight:600,boxSizing:"border-box",marginBottom:4,color:inst?theme.primary:"#94a3b8",background:"#fff"}}
                      />
                    : <input
                        style={{width:"100%",padding:"5px 7px",border:`1px solid ${theme.border}`,borderRadius:4,fontSize:11,color:theme.primary,fontWeight:600,marginBottom:4,boxSizing:"border-box"}}
                        value={inst} placeholder="Instructor"
                        onChange={e => upd(day, t, "instructor", e.target.value)}
                      />
                  }

                  {/* Subject dropdown — only assigned subjects, grouped GE / Major */}
                  {inst && subsList.length > 0
                    ? <select
                        style={{width:"100%",padding:"4px 6px",border:`1px solid ${theme.border}`,borderRadius:4,fontSize:11,boxSizing:"border-box",marginBottom:4,color:sub?"#0f172a":"#94a3b8",background:"#fff"}}
                        value={sub}
                    ...
                    : inst && subsList.length === 0
                      ? <div style={{fontSize:11,color:"#ef4444",padding:"4px 6px",marginBottom:4,background:"#fff0f0",borderRadius:4,border:"1px solid #fca5a5"}}>
                          ⚠ No subjects assigned for {activeSemester}
                        </div>
                      : <input
                          style={{width:"100%",padding:"5px 7px",border:`1px solid ${theme.border}`,borderRadius:4,fontSize:12,marginBottom:4,boxSizing:"border-box",opacity:inst?1:0.4}}
                          value={sub} placeholder={inst?"Subject":"Select instructor first"}
                          disabled={!inst}
                          onChange={e => upd(day, t, "subject", e.target.value)}
                        />
                  }
                  ...
                  {/* Room selector */}
                  <select
                    style={{width:"100%",padding:"4px 6px",border:`1px solid ${theme.border}`,borderRadius:4,fontSize:11,fontWeight:600,boxSizing:"border-box",background:sub?"#fff":theme.light,color:sub?"#0f172a":"#94a3b8",opacity:sub?1:0.35,cursor:sub?"pointer":"not-allowed"}}
                    value={room} disabled={!sub}
...
/* ════════ PRINT MODAL — FACULTY ════════ */
function PrintModal({ schedules, academicYear, semester, onClose, theme }) {
  const ref=useRef();
                        ...
                        {DAYS.map(day=>{
                          ...
                          return (
                            <td key={day} style={{border:"1px solid #ddd",textAlign:"center",verticalAlign:"middle",height:36,background:m?(lb?theme.light2:theme.light):"#fff"}}>
                              {m&&<><span style={{fontWeight:900,fontSize:10,display:"block"}}>{m.subject}</span>{m.section&&<span style={{fontSize:9,color:theme.primary,display:"block",fontWeight:700}}>{m.section}</span>}<span style={{fontSize:8,color:"#475569",display:"block"}}>{m.room}</span><span style={{fontSize:7,color:lb?theme.text:"#166534",fontWeight:700}}>{lb?"🔬 Lab":"📖 Lec"}</span></>}
                            </td>
                          );
                        })}
                      </tr>
...
/* ════════ PRINT MODAL — STUDENT ════════ */
function StudentPrintModal({ schedules, section, academicYear, semester, onClose, theme }) {
  const ref=useRef();
  const [printing,setPrinting]=useState(false);
  const real=schedules.filter(s=>!s.is_break);
  const total=real.reduce((s,c)=>s+(c.end-c.start),0);
                    ...
                    {DAYS.map(day=>{
                      ...
                      return (
                        <td key={day} style={{border:`1px solid #ddd`,textAlign:"center",verticalAlign:"middle",height:40,background:m?(lb?theme.light2:theme.light):"#fff"}}>
                          {m&&<><span style={{fontWeight:900,fontSize:10,display:"block"}}>{m.subject}</span>{m.instructor&&<span style={{fontSize:9,color:theme.primary,display:"block",fontWeight:700}}>{m.instructor}</span>}<span style={{fontSize:8,color:"#475569",display:"block"}}>{m.room}</span><span style={{fontSize:7,color:lb?theme.text:"#166534",fontWeight:700}}>{lb?"🔬 Lab":"📖 Lec"}</span></>}
                        </td>
                      );
                    })}
                  </tr>
...
function RoomPrintModal({ room, blocks, academicYear, semester, onClose, theme }) {
  const ref = useRef();
  const [printing, setPrinting] = useState(false);
                ...
                return (
                  <tr key={t}>
                    ...
                    {DAYS.map(day => {
                      ...
                      return (
                        <td key={day} style={{border:"1px solid #ddd",textAlign:"center",verticalAlign:"middle",height:40,background:m?(lb?theme.light2:theme.light):"#fff"}}>
                          {m&&<><span style={{fontWeight:900,fontSize:9,display:"block"}}>{m.subject}</span>{m.instructor&&<span style={{fontSize:8,color:theme.primary,display:"block",fontWeight:700}}>{m.instructor}</span>}{m.section&&<span style={{fontSize:8,color:"#475569",display:"block"}}>{m.section}</span>}<span style={{fontSize:7,color:lb?theme.text:"#166534",fontWeight:700}}>{lb?"🔬 Lab":"📖 Lec"}</span></>}
                        </td>
                      );
                    })}
                  </tr>
...
function RoomScheduleView({ instructorSchedules, studentSchedules, academicYear, semester, theme }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16,width:"100%",maxWidth:1300,alignSelf:"flex-start"}}>
      {printRoom && <RoomPrintModal room={printRoom} blocks={roomBlocksFor(printRoom)} academicYear={academicYear} semester={semester} onClose={()=>setPrintRoom(null)} theme={theme}/>}
                ...
                <div key={day} style={{marginBottom:14}}>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:8}}>
                    {dayBlocks.map((b,i) => {
                      const lab = isLab;
                      return (
                        <div key={i} style={{padding:"12px 16px",minWidth:180,border:`1px solid ${lab?theme.border:"#86efac"}`,borderLeft:`5px solid ${lab?theme.primary:"#16a34a"}`,borderRadius:8,background:lab?theme.light:"#f0fdf4",boxShadow:"0 1px 3px rgba(0,0,0,0.05)"}}>
                          <strong style={{fontSize:13,color:"#0f172a",display:"block",marginBottom:4}}>{b.subject}</strong>
                          {b.instructor && <span style={{fontSize:11,color:theme.primary,display:"block",fontWeight:700,marginBottom:2}}>👤 {b.instructor}</span>}
                          {b.section && <span style={{fontSize:11,color:"#475569",display:"block",fontWeight:600,marginBottom:2}}>🎓 {b.section}</span>}
                          <span style={{fontSize:12,color:"#334155",display:"block"}}>🕐 {fmtRange(Number(b.start), Number(b.end))}</span>
                          <span style={{fontSize:10,fontWeight:700,color:lab?theme.text:"#166534",background:lab?theme.light2:"#dcfce7",padding:"2px 8px",borderRadius:20,display:"inline-block",marginTop:6,border:`1px solid ${lab?theme.border:"#86efac"}`}}>{lab?"🔬 Laboratory":"📖 Lecture"}</span>
                        </div>
                      );
                    })}
                  </div>
...
function SubjectSetupPage({ theme, activeSemester }) {
  ...
  const [form,           setForm]           = useState({ subject_name:"", subject_type:"Major", year_level:1, units:3 });
  const [err,            setErr]            = useState("");
  const [saving,         setSaving]         = useState(false);
  ...
  function resetForm() {
    setForm({ subject_name:"", subject_type:"Major", year_level:1, units:3 });
    setEditId(null); setErr("");
  }

  function startEdit(s) {
    setEditId(s.id);
    setForm({ subject_name:s.subject_name, subject_type:s.subject_type, year_level:s.year_level, units:s.units });
    setErr("");
  ...
  const SubjectRow = ({ s, i }) => (
    <tr style={{ background: editId===s.id ? theme.light2 : (i%2===0?"#fff":"#f8fafc") }}>
      <td style={{padding:"10px 14px",borderBottom:`1px solid ${theme.light2}`,fontWeight:600,color:"#0f172a"}}>{s.subject_name}</td>
          ...
          {majorList.length === 0 ? (
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:theme.light}}>
                <th style={{padding:"9px 14px",textAlign:"left",borderBottom:`1px solid ${theme.light2}`,fontWeight:600,color:theme.text}}>Subject Name</th>
                <th style={{padding:"9px 12px",textAlign:"center",borderBottom:`1px solid ${theme.light2}`,fontWeight:600,color:theme.text,width:70}}>Units</th>
                <th style={{padding:"9px 12px",textAlign:"center",borderBottom:`1px solid ${theme.light2}`,fontWeight:600,color:theme.text,width:100}}>Actions</th>
              </tr></thead>
          ...
          {geList.length === 0 ? (
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:"#eff6ff"}}>
                <th style={{padding:"9px 14px",textAlign:"left",borderBottom:"1px solid #bfdbfe",fontWeight:600,color:"#1e40af"}}>Subject Name</th>
                <th style={{padding:"9px 12px",textAlign:"center",borderBottom:"1px solid #bfdbfe",fontWeight:600,color:"#1e40af",width:60}}>Units</th>
                <th style={{padding:"9px 12px",textAlign:"center",borderBottom:"1px solid #bfdbfe",fontWeight:600,color:"#1e40af",width:100}}>Actions</th>
              </tr></thead>
        ...
        <div style={{background:"#fff",borderRadius:10,overflow:"hidden",border:`1px solid ${theme.border}`}}>
          ...
          {majorList.length === 0 ? (
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:theme.light}}>
                <th style={{padding:"9px 14px",textAlign:"left",borderBottom:`1px solid ${theme.light2}`,fontWeight:600,color:theme.text}}>Subject Name</th>
                <th style={{padding:"9px 12px",textAlign:"center",borderBottom:`1px solid ${theme.light2}`,fontWeight:600,color:theme.text,width:60}}>Units</th>
                <th style={{padding:"9px 12px",textAlign:"center",borderBottom:`1px solid ${theme.light2}`,fontWeight:600,color:theme.text,width:100}}>Actions</th>
              </tr></thead>
              <tbody>{majorList.map((s,i)=><SubjectRow key={s.id} s={s} i={i}/>)}</tbody>
            </table>
      ...
      <div style={{background:"#fff",borderRadius:12,padding:24,boxShadow:`0 2px 10px rgba(0,0,0,0.07)`,borderTop:`4px solid ${theme.primary}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:12,alignItems:"end"}}>
          <div>
            ...
            <input style={inpStyle} placeholder="e.g. Purposive Communication" value={form.subject_name} onChange={e=>setForm(f=>({...f,subject_name:e.target.value}))}/>
          </div>
          <div>
...
/* ══════════════════════════════════════════════════════════════
   ...
   — Removed "Teaching Semester" field (managed via Academic Setup)
   — "Permanent" → "Regular"
   ══════════════════════════════════════════════════════════════ */
function InstructorPoolPage({ theme, activeSemester }) {
  ...
  const [loading,        setLoading]       = useState(true);
  const [editId,         setEditId]        = useState(null);
  const [form,           setForm]          = useState({ name:"", department:"", email:"", employment_type:"Regular" });
  const [err,            setErr]           = useState("");
  const [saving,         setSaving]        = useState(false);
  const [deleteConfirm,  setDeleteConfirm] = useState(null);

  // "Regular" (was Permanent) and "Part-time"
  ...
  function resetForm() {
    setForm({ name:"", department:"", email:"", employment_type:"Regular" });
    setEditId(null); setErr("");
  }

  function startEdit(inst) {
    setEditId(inst.id);
    // Normalize "Permanent" → "Regular" for existing data
    const empType = (inst.employment_type === "Permanent" ? "Regular" : inst.employment_type) || "Regular";
    setForm({ name:inst.name, department:inst.department||"", email:inst.email||"", employment_type:empType });
    setErr("");
    window.scrollTo({ top:0, behavior:"smooth" });
  }
  ...
  // Normalize employment type label for display (handle legacy "Permanent" in DB)
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
        <thead>
          <tr style={{background:theme.light}}>
            ...
            <th style={{padding:"9px 12px",textAlign:"left",borderBottom:`1px solid ${theme.light2}`,fontWeight:600,color:theme.text,width:90}}>Dept</th>
          ...
          {list.map((inst, i) => {
            ...
            return (
              <tr key={inst.id} style={{background: editId===inst.id ? theme.light2 : (i%2===0?"#fff":"#f8fafc")}}>
                <td style={{padding:"10px 16px",borderBottom:`1px solid ${theme.light2}`,fontWeight:600,color:"#0f172a"}}>{inst.name}</td>
                <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.light2}`,color:"#64748b",fontSize:12}}>{inst.department||"—"}</td>
                <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.light2}`,textAlign:"center"}}>
                  <span style={{padding:"3px 12px",borderRadius:20,fontSize:11,fontWeight:700,
                    background: isReg ? "#dcfce7" : "#fef9c3",
                    color:      isReg ? "#166534" : "#854d0e",
                    border:     `1px solid ${isReg ? "#86efac" : "#fde68a"}`}}>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:20,width:"100%",maxWidth:1100,alignSelf:"flex-start"}}>
      ...
      <div style={{background:"#fff",borderRadius:12,padding:24,boxShadow:`0 2px 10px rgba(0,0,0,0.07)`,borderTop:`4px solid ${theme.primary}`}}>
        ...
        {err && <div style={{background:"#fee2e2",color:"#dc2626",border:"1px solid #fca5a5",borderRadius:8,padding:"10px 14px",fontSize:13,marginBottom:14}}>⚠ {err}</div>}
        {/* 4-column grid — no Teaching Semester field */}
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:12,alignItems:"end"}}>
          <div>
          ...
          <div>
            ...
            <input style={inpStyle} placeholder="e.g. ICT" value={form.department} onChange={e=>setForm(f=>({...f,department:e.target.value}))}/>
          </div>
          <div>
...
/* ══════════════════════════════════════════════════════════════
   ...
   — Allows instructor to have BOTH GE and Major subjects
   — No longer restricts cross-type once one type is assigned
   ══════════════════════════════════════════════════════════════ */
function InstructorAssignmentPage({ theme, activeSemester }) {
  ...
  // Subjects already assigned to selected instructor
  const alreadyAssigned = new Set(
    assignments
      .filter(a => String(a.instructor_id) === form.instructor_id)
      .map(a => String(a.subject_id))
  );

  // ── Subject dropdown: show ALL subjects (GE + Major), only hide already-assigned ──
  // No cross-type restriction — instructors can hold GE + Major freely
  ...
  // ── Compute each instructor's subject-type group for badge only ──
  ...
  for (const a of filteredAssigns) {
    if (!byInstructor[a.instructor_name]) {
      byInstructor[a.instructor_name] = {
  ...
  // Normalize employment type display
  function displayEmpType(raw) {
    if (!raw || raw === "Permanent") return "Regular";
    return raw;
  }
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20,width:"100%",maxWidth:1100,alignSelf:"flex-start"}}>
      ...
      <div style={{background:"#f0fdf4",border:"1px solid #86efac",borderRadius:10,padding:"12px 18px",fontSize:13,color:"#166534",display:"flex",gap:10,alignItems:"center"}}>
        ...
        <span>
          ...
          An instructor can be assigned <strong>both GE and Major subjects</strong> — no restriction on type mixing.
        </span>
      </div>
      ...
      <div style={{background:"#fff",borderRadius:12,padding:24,boxShadow:`0 2px 10px rgba(0,0,0,0.07)`,borderTop:`4px solid ${theme.primary}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"2fr 2fr auto",gap:12,alignItems:"end"}}>
          <div>
            ...
            <select style={inpStyle} value={form.instructor_id} onChange={e=>setForm(f=>({...f,instructor_id:e.target.value,subject_id:""}))}>
              ...
              {instNone.length > 0 && (
                <optgroup label="📋 Not Yet Assigned">
                  {instNone.map(i=><option key={i.id} value={i.id}>
                    {displayEmpType(i.employment_type)==="Regular"?"🏛":"⏱"} {i.name}
                  </option>)}
                </optgroup>
              )}
              {instGEOnly.length > 0 && (
                <optgroup label="🌐 Currently GE Only">
                  {instGEOnly.map(i=><option key={i.id} value={i.id}>{i.name}</option>)}
                </optgroup>
              )}
              {instMajorOnly.length > 0 && (
                <optgroup label="🎯 Currently Major Only">
                  {instMajorOnly.map(i=><option key={i.id} value={i.id}>{i.name}</option>)}
                </optgroup>
              )}
              {instBoth.length > 0 && (
                <optgroup label="🔀 GE & Major">
                  {instBoth.map(i=><option key={i.id} value={i.id}>{i.name}</option>)}
                </optgroup>
              )}
            </select>
          </div>
          <div>
            <label style={{fontSize:12,fontWeight:600,color:"#374151",display:"block",marginBottom:5}}>
              Subject <span style={{color:"#64748b",fontWeight:400,fontSize:11}}>— GE and Major both available</span>
            </label>
            {/* ALL subjects shown, already-assigned shown as disabled */}
              ...
              {Object.entries(subjectsByYear).map(([yr, {ge, major}]) => {
                ...
                return (
                  <optgroup key={yr} label={`── Year ${yr} ──`}>
                    {geAvail.map(s=><option key={s.id} value={s.id}>🌐 {s.subject_name} (GE)</option>)}
                    {geUsed.map(s=><option key={s.id} value={s.id} disabled>✓ {s.subject_name} (GE — already assigned)</option>)}
                    {majorAvail.map(s=><option key={s.id} value={s.id}>🎯 {s.subject_name} (Major)</option>)}
        ...
        {instructors.length === 0 && (
          <div style={{marginTop:12,background:"#fef9c3",border:"1px solid #fde68a",borderRadius:8,padding:"10px 14px",fontSize:13,color:"#854d0e"}}>
            ⚠ No instructors found. Go to <strong>Instructor Pool</strong> to add instructors.
          </div>
        )}
        {subjects.length === 0 && (
          <div style={{marginTop:12,background:"#fef9c3",border:"1px solid #fde68a",borderRadius:8,padding:"10px 14px",fontSize:13,color:"#854d0e"}}>
            ⚠ No subjects found for {activeSemester}. Go to <strong>Subject Setup</strong> to add subjects.
          </div>
        )}
      </div>
      ...
      <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        ...
        <select value={filterInst} onChange={e=>setFilterInst(e.target.value)}
          style={{padding:"7px 12px",border:`1px solid ${theme.border}`,borderRadius:7,fontSize:13,background:"#fff",color:"#0f172a",outline:"none"}}>
          ...
          {[
            ...
            <button key={v} onClick={()=>setGroupFilter(v)}
              style={{padding:"5px 12px",borderRadius:20,border:"none",cursor:"pointer",fontSize:12,fontWeight:600,
                background: groupFilter===v ? col : bg,
                color:      groupFilter===v ? "#fff" : col,
                outline: groupFilter===v ? `2px solid ${col}` : "none"}}>
              {label}
            </button>
          ))}
        </div>
        <span style={{fontSize:12,color:"#64748b",marginLeft:4}}>
          {filteredByGroup.length} instructor{filteredByGroup.length!==1?"s":""}
        </span>
      </div>
      ...
      {loading ? (
        ...
        <div style={{textAlign:"center",padding:"40px 0",color:"#94a3b8",fontSize:14}}>
          {assignments.length===0 ? `No assignments yet for ${activeSemester}.` : "No assignments match the filter."}
        </div>
      ) : (
          ...
          return (
            <div key={instName} style={{background:"#fff",borderRadius:12,overflow:"hidden",boxShadow:`0 2px 8px rgba(0,0,0,0.06)`}}>
              <div style={{background:theme.primary,color:"#fff",padding:"10px 20px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                ...
                <span style={{padding:"2px 10px",borderRadius:20,fontSize:11,fontWeight:700,...groupBadgeStyle}}>
                  {groupBadge}
                </span>
                <span style={{marginLeft:"auto",fontSize:12,opacity:0.8}}>
                  {items.length} subject{items.length!==1?"s":""} · {totalUnits} unit{totalUnits!==1?"s":""}
                </span>
              </div>
              <div style={{padding:"14px 20px"}}>
                {geItems.length === 0 && majItems.length === 0 ? (
                  ...
                  <div style={{display:"grid",gridTemplateColumns:geItems.length>0&&majItems.length>0?"1fr 1fr":"1fr",gap:16}}>
                    {geItems.length > 0 && (
                      <div>
                        <div style={{fontSize:12,fontWeight:700,color:"#1d4ed8",marginBottom:8,background:"#dbeafe",border:"1px solid #bfdbfe",borderRadius:6,padding:"5px 12px",display:"inline-block"}}>
                          🌐 GE Subjects — {geItems.length}
                        </div>
                        <div style={{display:"flex",flexDirection:"column",gap:6}}>
                          {geItems.map(a => (
                    ...
                    {majItems.length > 0 && (
                      <div>
                        <div style={{fontSize:12,fontWeight:700,color:theme.text,marginBottom:8,background:theme.light2,border:`1px solid ${theme.border}`,borderRadius:6,padding:"5px 12px",display:"inline-block"}}>
                          🎯 Major Subjects — {majItems.length}
                        </div>
                        <div style={{display:"flex",flexDirection:"column",gap:6}}>
                          {majItems.map(a => (
      ...
      <div style={{marginBottom:16,padding:"10px 14px",background:"rgba(255,255,255,0.07)",borderRadius:8,border:"1px solid rgba(255,255,255,0.1)"}}>
        {auth?.isSuperAdmin && previewDept ? (
          <>
            ...
            <button onClick={()=>setPreviewDept(null)} style={{width:"100%",padding:"6px",background:"rgba(239,68,68,0.15)",color:"#fca5a5",border:"1px solid rgba(239,68,68,0.2)",borderRadius:6,cursor:"pointer",fontSize:11,fontWeight:600}}>
...
/* ════════ PAGE CONTENT ════════ */
function PageContent({ activePage, data, setData, theme, deptCode }) {
  const auth = useAuth();
  ...
  const [toast,setToast]=useState(null);      // conflicts array
  ...
  if (activePage==="Academic Setup") {
    ...
    return (
      <div style={cardStyle}>
        <SchoolHeader compact academicYear={data.academicYear} semester={data.semester} theme={theme}/>
        <h3 style={{fontSize:18,fontWeight:700,margin:0,color:"#0f172a"}}>Academic Setup</h3>
        {(data.academicYear || data.semester) && (
          <div style={{background:theme.light2,border:`1px solid ${theme.border}`,borderRadius:8,padding:"10px 16px",fontSize:13,color:theme.text}}>
            Currently saved: <strong>A.Y. {data.academicYear||"—"}</strong> · <strong>{data.semester||"—"}</strong>
          </div>
        )}
        <label style={{fontSize:13,fontWeight:600,color:"#374151"}}>Academic Year</label>
        <input placeholder="e.g. 2024–2025" style={inpStyle} value={ay} onChange={e=>setAy(e.target.value)}/>
        <label style={{fontSize:13,fontWeight:600,color:"#374151"}}>Semester</label>
        <select style={inpStyle} value={sem} onChange={e=>setSem(e.target.value)}>
          <option>1st Semester</option>
          <option>2nd Semester</option>
        </select>
        <div style={{background:"#fef9c3",border:"1px solid #fde68a",borderRadius:8,padding:"10px 14px",fontSize:13,color:"#854d0e"}}>
          ⚠ Changing the semester will update which subjects, instructors, and assignments appear across all setup pages.
        </div>
      ...
      <div style={cardStyle}>
        {toast && (
          <ConflictToast
            ...
            allSchedules={allRealSchedules}
            onClose={()=>setToast(null)}
            onMoveSchedule={null} /* student schedules don't have ID-based move yet */
          />
        )}
  ...
  if (activePage==="Schedule Output") {
    ...
    return (
      <div style={cardStyle}>
        {showPrint&&<PrintModal schedules={data.schedules} academicYear={data.academicYear} semester={data.semester} onClose={()=>setShowPrint(false)} theme={theme}/>}
        {showStudentPrint&&<StudentPrintModal schedules={data.studentSchedules.filter(s=>s.section===showStudentPrint)} section={showStudentPrint} academicYear={data.academicYear} semester={data.semester} onClose={()=>setShowStudentPrint(null)} theme={theme}/>}
        ...
        {(!data.academicYear || !data.semester) && (
          <div style={{background:"#fef9c3",border:"1px solid #fde68a",borderRadius:8,padding:"10px 16px",fontSize:13,color:"#854d0e",display:"flex",gap:8,alignItems:"center"}}>
            <span>⚠</span>
            <span>Academic Year or Semester not set. Go to <strong>Academic Setup</strong> to configure.</span>
          </div>
        )}
                    ...
                    <div key={day} style={{marginBottom:14}}>
                      ...
                      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:8}}>
                        {dc.map((c,i)=>{
                          ...
                          if(c.is_break) return <div key={i} style={{padding:"10px 14px",minWidth:130,border:"1px solid #fde68a",borderLeft:"5px solid #f59e0b",borderRadius:8,background:"#fefce8"}}><span style={{fontSize:12,fontWeight:700,color:"#854d0e"}}>☕ Break</span><span style={{fontSize:11,color:"#92400e",display:"block",marginTop:3}}>🕐 {fmtRange(c.start,c.end)}</span></div>;
                          return (
                            <div key={i} style={{padding:"12px 16px",minWidth:165,border:`1px solid ${lab?theme.border:"#86efac"}`,borderLeft:`5px solid ${lab?theme.primary:"#16a34a"}`,borderRadius:8,background:lab?theme.light:"#f0fdf4",boxShadow:"0 1px 3px rgba(0,0,0,0.05)"}}>
                              <strong style={{fontSize:13,color:"#0f172a",display:"block",marginBottom:5}}>{c.subject}</strong>
                              {c.section&&<span style={{fontSize:11,color:theme.primary,display:"block",fontWeight:700,marginBottom:3}}>🎓 {c.section}</span>}
                              ...
                              <span style={{fontSize:11,fontWeight:700,color:lab?theme.text:"#166534",background:lab?theme.light2:"#dcfce7",padding:"2px 10px",borderRadius:20,display:"inline-block",marginTop:7,border:`1px solid ${lab?theme.border:"#86efac"}`}}>{lab?"🔬":"📖"} {c.roomType||"Lecture"}</span>
                              {c.id&&!c.is_break&&<div><button onClick={()=>setEditBlock(c)} style={{marginTop:8,padding:"5px 12px",background:theme.light,color:theme.primary,border:`1px solid ${theme.border}`,borderRadius:6,cursor:"pointer",fontSize:11}}>✏ Edit</button></div>}
                            </div>
                          );
                        })}
                      </div>
                      ...
                      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:8}}>
                        {dc.map((c,i)=>{
                          ...
                          return (
                            <div key={i} style={{padding:"12px 16px",minWidth:165,border:`1px solid ${lab?theme.border:"#86efac"}`,borderLeft:`5px solid ${lab?theme.primary:"#16a34a"}`,borderRadius:8,background:lab?theme.light:"#f0fdf4",boxShadow:"0 1px 3px rgba(0,0,0,0.05)"}}>
                              <strong style={{fontSize:13,color:"#0f172a",display:"block",marginBottom:5}}>{c.subject}</strong>
                              {c.instructor&&<span style={{fontSize:11,color:theme.primary,display:"block",fontWeight:700,marginBottom:3}}>👤 {c.instructor}</span>}
                              <span style={{fontSize:12,color:"#334155",display:"block"}}>🕐 {fmtRange(c.start,c.end)}</span>
                              {c.room&&<span style={{fontSize:12,color:"#64748b",display:"block",marginTop:2}}>📍 {c.room}</span>}
                              <span style={{fontSize:11,fontWeight:700,color:lab?theme.text:"#166534",background:lab?theme.light2:"#dcfce7",padding:"2px 10px",borderRadius:20,display:"inline-block",marginTop:7,border:`1px solid ${lab?theme.border:"#86efac"}`}}>{lab?"🔬":"📖"} {c.roomType||"Lecture"}</span>
                            </div>
    ...
    <div style={{display:"flex",height:"100vh",fontFamily:"'Segoe UI',sans-serif",background:theme.light}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{background:"#fff",padding:"14px 28px",fontSize:18,fontWeight:700,color:"#0f172a",display:"flex",alignItems:"center",borderBottom:`2px solid ${theme.light2}`}}>
          ...
          {data.semester && (
            <span style={{marginLeft:12,background:"#fefce8",color:"#854d0e",border:"1px solid #fde68a",borderRadius:20,padding:"2px 10px",fontSize:11,fontWeight:600}}>
              📅 {data.semester}
            </span>
          )}
          {data.academicYear && (
            <span style={{marginLeft:6,background:theme.light2,color:theme.text,border:`1px solid ${theme.border}`,borderRadius:20,padding:"2px 10px",fontSize:11,fontWeight:600}}>
              A.Y. {data.academicYear}
            </span>
          )}
          {!auth?.isSuperAdmin && (
        ...
        <div style={{flex:1,padding:28,overflowY:"auto",display:"flex",justifyContent:"center"}}>
          <PageContent
            ...
            setData={setData}
            theme={theme}
            deptCode={auth?.isSuperAdmin ? previewDept : (auth?.deptCode || null)}
          />
        </div>
      </div>
    </div>
